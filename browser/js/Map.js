...
require([
	...
	"dojo/domReady!"
		...
		map = new Map("map", {
			basemap: "streets",
			center: [-122.45, 37.75], // longitude, latitude (starts out in San Francisco, CA until user clicks locate button to go to their location)
			zoom: 13,
			infoWindow: popup
		});
	...
	var popupForm = 
		`<form id=popupForm>
			...
			<h5>Telephone</h5><input id="telephone" type='text' value='' pattern='^[\\d() +-]+$'/></br>
			<h5>Email</h5><input id="email" type='text' value='' pattern='^[_A-Za-z0-9-\\+]+(\\.[_A-Za-z0-9-]+)*@[A-Za-z0-9-]+(\\.[A-Za-z0-9]+)*(\\.[A-Za-z]{2,})$'/></br>
			<h5>Blood Type</h5>
			<select>
